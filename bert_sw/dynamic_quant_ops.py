...
def tensor_quant_scale(arr, scale=None, bits=8):
    ...
    if scale is None:
        ...
        scale = max(abs(arr_min / symmetric_qmin),
                    abs(arr_max / symmetric_qmax))

    arr_q = torch.round(
        (torch.clip((arr / scale), min=-2**(bits-1)-1, max=2**(bits-1)-1)))
...
def quant_scale(arr, scale=None, dtype=np.int8):
    ...
    arr_q = np.rint((arr / scale).clip(-2**(bits-1) -
                    1, 2**(bits-1)-1)).astype(np.int32)
    return arr_q, scale
